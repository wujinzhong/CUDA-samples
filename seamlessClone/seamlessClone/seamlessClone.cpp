...
public:
	SeamlessClone( cudaStream_t stream, 
			int multiProcessorCount,
                	int maxThreadsPerMultiProcessor ): mStream(stream), 
	mSM(multiProcessorCount), mMaxThdPerSM(maxThreadsPerMultiProcessor) 
	{
		assert(mStream);
        myNpp.setStream(mStream);
	...
	void init_resize();
	void run();
	void seamlessCloneGPU( Mat dst, Mat patch, Mat mask, Point point, Mat& blend, int flag );
private:
...
void SeamlessClone::seamlessCloneGPU( Mat dst, Mat patch, Mat mask, Point point, Mat& blend, int flag )
{
