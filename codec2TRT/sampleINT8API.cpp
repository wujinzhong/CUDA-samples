...
public:
    ...
    //!
    Logger::TestResult build( int iFp32fp16int8 );
    Logger::TestResult post_build();
    //!
    //! \brief Runs the TensorRT inference engine for this sample
    //!
    Logger::TestResult infer(int threadIdx, int batchSize);
    Logger::TestResult infer(int threadIdx, int batchSize, float* image, int nWidth, int nHeight, int nChannel, cudaStream_t stream, bool bLocateDevice=true);
    void createExecutionContexts(int nThreads, int threadIdx);
    ...
    std::vector<nvinfer1::IExecutionContext*> m_execCtxList;
    std::vector<samplesCommon::BufferManager *> mBuffersList;
private:
    //std::shared_ptr<nvinfer1::ICudaEngine> mEngine{nullptr}; //!< The TensorRT engine used to run the network

    //std::vector<SampleSharedPtr<nvinfer1::IExecutionContext>> m_execCtxList;
    //std::vector<samplesCommon::BufferManager *> mBuffersList;
    ...
    bool prepareInput_copy2buffers(samplesCommon::BufferManager& buffers, int batchSize, float* img, int imgWidth, int imgHeight, int imgChannel, cudaStream_t stream);
    //!
    //! \brief Verifies that the output is correct and prints it
    //!
    bool verifyOutput(const samplesCommon::BufferManager& buffers, int bs) const;
    ...
    //!
    void setLayerPrecision(SampleUniquePtr<nvinfer1::INetworkDefinition>& network, int iFp32fp16int8);
...
{
    for( int i=0; i<mBuffersList.size(); i++ )
    {
	samplesCommon::BufferManager * buffers = mBuffersList[i];
    	if(buffers!=NULL)
	    delete buffers;
	mBuffersList[i] = NULL;
    }
    for( int i=0; i<m_execCtxList.size(); i++ )
    {
	if( !m_execCtxList[i] )
	    m_execCtxList[i]->destroy();
	m_execCtxList[i]=NULL;
    }
}
...
//! \brief  Sets computation precision for network layers
//!
void SampleINT8API::setLayerPrecision(SampleUniquePtr<nvinfer1::INetworkDefinition>& network, int iFp32fp16int8)
{
    auto dt = nvinfer1::DataType::kHALF;
    if( iFp32fp16int8==2 ) dt = nvinfer1::DataType::kHALF;
    if( iFp32fp16int8==3 ) dt = nvinfer1::DataType::kINT8;
    ...
    for (int i = 0; i < network->getNbLayers(); ++i)
    {
        ...
        if (mParams.verbose)
        {
            ...
            gLogInfo << "Layer: " << layerName << ". Precision: ?" <<std::endl;
        }
        ...
        for (int j = 0; j < layer->getNbOutputs(); ++j)
        {
            ...
            {
                std::string tensorName = layer->getOutput(j)->getName();
                gLogInfo << "Tensor: " << tensorName << ". OutputType: ?" << std::endl;
            }
            // set output type of the tensor
            layer->setOutputType(j, dt);
        }
    }
}
...
bool SampleINT8API::prepareInput_copy2buffers(samplesCommon::BufferManager& buffers,
int batchSize, float* img, int imgWidth, int imgHeight, int imgChannel, cudaStream_t stream)
{
    int channels = mParams.mPreproc.inputDims.at(0);
    int height = mParams.mPreproc.inputDims.at(1);
    int width = mParams.mPreproc.inputDims.at(2);
    assert( channels==imgChannel && height==imgHeight && width==imgWidth );
    float* deviceInputBuffer = static_cast<float*>(buffers.getDeviceBuffer(mInOut["input"]));
    int copySz = sizeof(float)*width*height*channels*batchSize;
    assert( buffers.size(mInOut["input"])>=copySz );
    CUDA_CHECK( cudaMemcpyAsync(deviceInputBuffer, img, copySz, cudaMemcpyDeviceToDevice, stream) );
...
//! \brief Verifies that the output is correct and prints it
//!
bool SampleINT8API::verifyOutput(const samplesCommon::BufferManager& buffers, int bs) const
{
    ...
    printf("verifyOutput for batchSize(%d)\n", mParams.batchSize);
    int sz = output.size()/mParams.batchSize;
    for( int i=0; i<mParams.batchSize; i++ )
    {
	 vector<float> outputI(output.data()+sz*i, output.data()+sz*(i+1));
	 auto inds = samplesCommon::argsort(outputI.cbegin(), outputI.cend(), true);
    	...
    	vector<string> top5Result = samplesCommon::classify(referenceVector, outputI, 5);

    	gLogInfo << "SampleINT8API result: Detected:" << std::endl;
    	if( i==0 )//only show first frame's result, for simplification.
	for (int i = 1; i <= 5; ++i)
    	{
...
//!
Logger::TestResult SampleINT8API::build( int iFp32fp16int8 )
{
    ...
    if( iFp32fp16int8==2 ) config->setFlag(BuilderFlag::kFP16);
    if( iFp32fp16int8==3 ) config->setFlag(BuilderFlag::kINT8);
    ...
    auto maxBatchSize = mParams.maxBatchSize;
    ...
    printf("builder->setMaxBatchSize(%d)\n", maxBatchSize);
    builder->setMaxBatchSize(maxBatchSize);

    // force layer to execute with required precision
    config->setFlag(BuilderFlag::kSTRICT_TYPES);
    setLayerPrecision(network, iFp32fp16int8);
...
Logger::TestResult SampleINT8API::post_build()
{
...
void SampleINT8API::createExecutionContexts(int nThreads, int threadIdx)
{
	assert(mEngine);
	assert( m_execCtxList.size()==nThreads && threadIdx<nThreads );
	//for( int i=0; i<nThreads; i++ )
	{
		m_execCtxList[threadIdx] = mEngine->createExecutionContext();
	}
	return;
}
...
//!
Logger::TestResult SampleINT8API::infer(int threadIdx, int batchSize, float* image, int nWidth, int nHeight, int nChannel, cudaStream_t stream, bool bLocateDevice)
{
    mParams.batchSize = batchSize;
    assert( mParams.batchSize<=mParams.maxBatchSize );
    // Create RAII buffer manager object
    if( !mBuffersList[threadIdx] )
    {
    	mBuffersList[threadIdx] = new samplesCommon::BufferManager(mEngine, mParams.maxBatchSize);
    }
    samplesCommon::BufferManager *buffers = mBuffersList[threadIdx];

    //auto context = m_execCtx;
    //auto context = SampleUniquePtr<nvinfer1::IExecutionContext>(mEngine->createExecutionContext());
    if( !m_execCtxList[threadIdx] )
    {
	printf("m_ecList[%d] is null!\n", threadIdx);
        return Logger::TestResult::kFAILED;
    }
    ...
    {
	printf( "if (!prepareInput(buffers)) failed\n" );
        return Logger::TestResult::kFAILED;
    }
    ...
    assert( bLocateDevice==true );
    if( image==NULL )
    {
	printf("input image is NULL, use default image instead!\n");
    }
    else
    {
    	if( !prepareInput_copy2buffers(*buffers, batchSize, image, nWidth, nHeight, nChannel, stream) )
    	{
		printf("if( !prepareInput_copy2buffers(buffers, image, nWidth, nHeight, nChannel, stream) ) failed\n");
        	return Logger::TestResult::kFAILED;
    	}
    }
    // Asynchronously enqueue the inference work
    if(!m_execCtxList[threadIdx]->enqueue(mParams.batchSize, buffers->getDeviceBindings().data(), stream, nullptr))
    {
        return Logger::TestResult::kFAILED;
    }
    ...
    auto ret = verifyOutput(*buffers, batchSize) ? Logger::TestResult::kRUNNING : Logger::TestResult::kFAILED;
    return ret;
}

Logger::TestResult SampleINT8API::infer(int threadIdx, int batchSize)
{
    mParams.batchSize = batchSize;
    assert( mParams.batchSize<=mParams.maxBatchSize );
    // Create RAII buffer manager object
    //samplesCommon::BufferManager buffers(mEngine, mParams.maxBatchSize);
    if( !mBuffersList[threadIdx] )
    {
        mBuffersList[threadIdx] = new samplesCommon::BufferManager(mEngine, mParams.maxBatchSize);
    }
    ...
    // Check and print the output of the inference
    return verifyOutput(*buffers, batchSize) ? Logger::TestResult::kRUNNING : Logger::TestResult::kFAILED;
}
...
bool parseSampleINT8APIArgs(SampleINT8APIArgs& args, int argc, char* argv[])
{
    for (int i = 1; i < argc; ++i)
    {
        ...
        else if (!strncmp(argv[i], "--maxBatchSize=", 15))
        {
            args.maxBatchSize = std::stoi(argv[i] + 15);
        }
...
SampleINT8APIParams initializeSampleParams(SampleINT8APIArgs args)
{
    ...
    params.maxBatchSize = args.maxBatchSize;
...
void printHelpInfo()
{
    ...
    std::cout << "--maxBatchSize=1/2/4/8/etc"<<std::endl;
...
int sampleINT8API_createExecutionContexts( SampleINT8API*sample, int nThreads, int threadIdx )
{
	sample->createExecutionContexts(nThreads, threadIdx);
	return 0;
}

void saveEngine( std::string saveFile, std::shared_ptr<nvinfer1::ICudaEngine>& engine )
{
	std::ofstream p(saveFile, std::ios::binary | std::ios::out);
	nvinfer1::IHostMemory* ptr = engine->serialize();
        assert(ptr);
        p.write(reinterpret_cast<const char*>(ptr->data()), ptr->size());
        ptr->destroy();
        p.close();
        printf("TRT Engine file saved to: %s\n", saveFile.c_str());
}

std::shared_ptr<nvinfer1::ICudaEngine> readEngine( std::string saveFile )
{
    std::ifstream engineFile(saveFile, std::ios::binary | std::ios::in);
    if (!engineFile)
    {
        return nullptr;
    }
    ...
    return std::shared_ptr<nvinfer1::ICudaEngine>(infer->deserializeCudaEngine(engineData.data(), fsize, nullptr), samplesCommon::InferDeleter());
}

int sampleINT8API_buildEngine(int argc, char** argv, SampleINT8API**sample )
{
    ...
    *sample = new SampleINT8API(params);
    gLogInfo << "Building and running a INT8 GPU inference engine for " << params.modelFileName << std::endl;
   
    std::string sFp32fp16int8 = "_fp32";
    int iFp32fp16int8 = 1;
    if( args.runInFp16 ) { iFp32fp16int8 = 2; sFp32fp16int8 = "_fp16"; }
    if( args.runInInt8 ) { iFp32fp16int8 = 3; sFp32fp16int8 = "_int8"; }
     
    std::stringstream ss; ss<<params.modelFileName<<"_bs"<<params.maxBatchSize<<sFp32fp16int8<<".plan";
    std::string saveEngineFile = ss.str();
    printf( "%s\n", saveEngineFile.c_str() );
    ifstream fileRead;
    fileRead.open( saveEngineFile );
    if(!fileRead)
    {
	printf("no plan file %s exist!\n", saveEngineFile.c_str());
	fileRead.close();
	auto buildStatus = (*sample)->build( iFp32fp16int8 );

    	if (buildStatus == Logger::TestResult::kWAIVED)
    	{
        	return -1;//gLogger.reportWaive(sampleTest);
    	}
    	else if (buildStatus == Logger::TestResult::kFAILED)
    	{
        	return -1;//gLogger.reportFail(sampleTest);
    	}
	printf("engine built!\n");
	saveEngine( saveEngineFile, (*sample)->mEngine );
	printf("engine saved!\n");	
    }
    else
    {
	(*sample)->mEngine = readEngine( saveEngineFile );
	(*sample)->post_build();
	printf("engine read from %s!\n", saveEngineFile.c_str());
    }
}


int sampleINT8API_destroy(SampleINT8API*sample)
{
    ...
    return 0; //gLogger.reportPass(sampleTest);
}


int sampleINT8API_infer(SampleINT8API*sample, int threadIdx, int bs, float* image, int nWidth, int nHeight, int nChannel, cudaStream_t stream )
{
    auto ret = sample->infer(threadIdx, bs, image, nWidth, nHeight, nChannel, stream);

    if (ret != Logger::TestResult::kRUNNING)
    {
        return -1;//gLogger.reportFail(sampleTest);
    }
...
/*
int sampleINT8API_main(int argc, char** argv, SampleINT8API**sample )
{
    sampleINT8API_buildEngine( argc, argv, sample );
    sampleINT8API_createExecutionContexts(*sample, 1);

    cudaStream_t stream;
    CHECK(cudaStreamCreate(&stream));
    int bs = 1;
    sampleINT8API_infer( *sample, 0, bs, NULL, 224, 224, 3, stream );
