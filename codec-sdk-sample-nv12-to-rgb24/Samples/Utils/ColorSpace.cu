...
template<class YuvUnitx2, class Rgb, class RgbIntx2>
__global__ static void YuvToRgb24Kernel(uint8_t *pYuv, int nYuvPitch, uint8_t *pRgb, int nRgbPitch, int nWidth, int nHeight) {
    int x = (threadIdx.x + blockIdx.x * blockDim.x) * 2;
    int y = (threadIdx.y + blockIdx.y * blockDim.y) * 2;
    if (x + 1 >= nWidth || y + 1 >= nHeight) {
        return;
    ...
    Rgb p00 = YuvToRgbForPixel<Rgb>(l0.x, ch.x, ch.y);
    Rgb p01 = YuvToRgbForPixel<Rgb>(l0.y, ch.x, ch.y);
    Rgb p10 = YuvToRgbForPixel<Rgb>(l1.x, ch.x, ch.y);
    Rgb p11 = YuvToRgbForPixel<Rgb>(l1.y, ch.x, ch.y);

    // just for testing
    //p00.c.r = 255; p00.c.g = 0  ; p00.c.b = 255; p01.c.r = 0  ; p01.c.g = 0  ; p01.c.b = 255;
    //p10.c.r = 0  ; p10.c.g = 255; p10.c.b = 0  ; p11.c.r = 255; p11.c.g = 255; p11.c.b = 0  ;
...
void Nv12ToColor24(uint8_t *dpNv12, int nNv12Pitch, uint8_t *dpBgra, int nBgraPitch, int nWidth, int nHeight, int iMatrix) {
    SetMatYuv2Rgb(iMatrix);
    YuvToRgb24Kernel<uchar2, COLOR24, uint2>
        <<<dim3((nWidth + 63) / 32 / 2, (nHeight + 3) / 2 / 2), dim3(32, 2)>>>
        (dpNv12, nNv12Pitch, dpBgra, nBgraPitch, nWidth, nHeight);
}
...
template void Nv12ToColor24<BGR24>(uint8_t *dpNv12, int nNv12Pitch, uint8_t *dpBgra, int nBgraPitch, int nWidth, int nHeight, int iMatrix);
template void Nv12ToColor24<RGB24>(uint8_t *dpNv12, int nNv12Pitch, uint8_t *dpBgra, int nBgraPitch, int nWidth, int nHeight, int iMatrix);
